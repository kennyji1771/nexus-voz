...
from nexus_vox.data.audio import stream_audio_chunks
from nexus_vox.default_parameters import (
...
from nexus_vox.modules.stateful_module import ModelState
from nexus_vox.utils.logging_utils import enable_logging
from nexus_vox.utils.utils import _ORIGINS_OF_PREDEFINED_VOICES

logger = logging.getLogger(__name__)

cli_app = typer.Typer(
    help="Nexus.Vox - Text-to-Speech generation tool", pretty_exceptions_show_locals=False
)


# ------------------------------------------------------
# The nexus-vox server implementation
# ------------------------------------------------------
...
web_app = FastAPI(
    title="Nexus.Vox API", description="Text-to-Speech generation API", version="1.0.0"
)
...
def _loaded_model() -> TTSModel:
    if tts_model is None:
        raise RuntimeError("no model loaded: `nexus-vox serve` loads it before serving requests")
    return tts_model
...
# ------------------------------------------------------
# The nexus-vox single generation CLI implementation
# ------------------------------------------------------
...
def generate(
    ...
    """Generate speech using Nexus.Vox."""
    ...
    with enable_logging("pocket_tts", log_level):
        ...
        logger.info(
            "If you like Nexus.Vox, star us at https://github.com/kennyji1771/nexus-voz"
        )
